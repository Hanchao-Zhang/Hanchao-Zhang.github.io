...
For an given orthogonal matrix $\mathbf B$, the projection index of a given PSD matrix $\mathbf \\mathbf \psi$, denoted $\bLambda_{\mathbf B}(\mathbf \\mathbf \psi): \S_+^p \to   \mathbb D_+^p$, is defined as:
\begin{align}\label{sec2:eq:projectionlambda}
		\bLambda_{\mathbf B}(\\mathbf \psi) = \arg\min_{\Theta  \in \mathbb D_+^p}\Vert  \\mathbf \psi - \mathbf B \mathbf \Theta \mathbf B^\intercal  \Vert^2
\end{align} 
