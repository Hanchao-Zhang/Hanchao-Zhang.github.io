...
- we can find $\mathbf B$ by
\begin{align*}
    \mathbf B = \underset{\mathbf B \in \mathcal V_q(\mathbb R^p)}{\text{argmin}} \mathcal E \big\Vert \mathbf \Psi - \mathbf B \big() \left(\mathbf B^T \mathbf \Psi \mathbf B\right) \circ \mathbf I \big) \mathbf B^T \big\Vert_F^2
\end{align*}
