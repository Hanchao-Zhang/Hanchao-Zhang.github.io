...
:::{.incremental}
- let $\mathbf \Psi$ be a random positive semi-definite matrix
...
\begin{definition}\label{sec3:def:projectionindex}
%\thad{[Does this modified definition capture your original definition?]} 


For an given orthogonal matrix $\mathbf B$, the projection index of a given PSD matrix $\mathbf \bpsi$, denoted $\bLambda_{\mathbf B}(\mathbf \bpsi): \S_+^p \to   \mathbb D_+^p$, is defined as:
\begin{align}\label{sec2:eq:projectionlambda}
		\bLambda_{\mathbf B}(\bpsi) = \arg\min_{\Theta  \in \mathbb D_+^p}\Vert  \bpsi - \mathbf B \mathbf \Theta \mathbf B^\intercal  \Vert^2
\end{align} 
\end{definition}
