...
where 
\begin{align} 
    \mathcal T(\mathbf \Lambda) = \mathbf B(\mathbf \Psi) \mathbf \Lambda \mathbf B^\intercal(\mathbf \Psi) \qquad \forall \mathbf \Lambda \in \mathbb D^+
\end{align}
defines the princpal and self-consistent tensors which is also the common principal component subspace
:::
